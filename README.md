...
- 파일 트리, 분석용 스니펫, 엔트리 포인트 후보 인덱싱
- OpenAI 기반 프로젝트 요약 생성
- 프로젝트 종류 감지 후 스택별 헬스 스코어링
- 최근 커밋 히스토리 요약
...
[server/api/analyze.post.ts](./server/api/analyze.post.ts)에서 저장소의 최신 analysis 레코드를 `PENDING` 상태로 준비하고,
[server/services/analysis-job.service.ts](./server/services/analysis-job.service.ts)에 작업을 등록합니다.  
[server/plugins/analysis-worker.server.ts](./server/plugins/analysis-worker.server.ts)의 워커가 큐를 가져와 [server/services/analysis.service.ts](./server/services/analysis.service.ts)의 실제 분석을 실행합니다.

분석 단계:

1. `RUNNING` 상태로 전환
2. Git clone 및 파일/커밋 스캔
3. 파일트리와 스니펫으로 프로젝트 종류 감지
4. 감지된 종류에 맞는 헬스 스코어 기준 생성
5. OpenAI로 프로젝트 요약 생성
6. 파일 인덱스와 커밋 요약 저장
7. `SUCCESS` 또는 `FAILED` 상태 반영
8. 항목별 분석 요청 시 `analysis_artifact`에 실행 상태와 OpenAI 결과 JSON 저장
9. 실패한 큐 작업은 `analysis_job`에서 재시도 또는 실패 상태로 관리
...
- 추론된 기술 스택
- 엔트리 포인트 목록
- 저장소 건강도 점수: Nuxt, Node, Java, Python, Go, Rust 등 감지된 프로젝트 종류별 기준 적용
- 항목별 상세 리포트
...
- `analysis_artifact`: 항목별 분석 실행 상태와 결과 JSON
- `analysis_job`: 기본 분석과 항목별 분석을 처리하는 백그라운드 작업 큐
...
- [server/utils/project-health.ts](./server/utils/project-health.ts): 프로젝트 종류 감지와 스택별 헬스 스코어 기준
...
- [pages/analysis/[id]/index.vue](./pages/analysis/[id]/index.vue): 분석 결과 페이지
...
### `POST /api/analysis/:id/items`

분석 결과 화면의 항목별 상세 분석을 시작합니다.
...
항목 실행 상태는 `analysis_artifact`에 저장되며, 완료된 항목은 별도 상세 페이지에서 확인합니다.
...
- 공용 Nginx용 Atlas 서버 블록: [deploy/frontend/conf.d/atlas.conf](./deploy/frontend/conf.d/atlas.conf)
...
- `FRONT_NGINX_APP_PATH` (선택, 기본값 `/home/ubuntu/app/frontend`, 공용 Nginx Compose 경로)

배포 서버에는 Docker와 Docker Compose 플러그인이 설치되어 있어야 합니다. 또한 기존 공용 Nginx가 `conf.d` 디렉터리를 include하도록 구성되어 있어야 하며, Atlas 배포는 [deploy/frontend/conf.d/atlas.conf](./deploy/frontend/conf.d/atlas.conf)를 공용 Nginx의 `conf.d/atlas.conf`로 전송한 뒤 Nginx 컨테이너를 재생성합니다.

주의: 공용 Nginx의 기본 마운트 구조는 Home 프론트 배포가 만들지만, Atlas 도메인 server block 원본은 이 저장소의 `atlas.conf`가 소유합니다. 이 분리가 깨지면 `atlas.glassworld.co.kr` 요청이 기존 `home.glassworld.co.kr` 프로젝트로 라우팅될 수 있습니다.
...
- [pages/analysis/[id]/items/[type].vue](./pages/analysis/[id]/items/[type].vue)
